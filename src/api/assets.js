...
class Assets {
   ...
   */
    getAsset(assetId) {
        return new Promise((resolve, reject) => {
            ...
            getRequest(
                `${this._settings.apiEndpoint}/assets/${encodeURIComponent(assetId)}`,
   ...
   */
    getAssets(params = {}) {
        return new Promise((resolve, reject) => {
            getRequest(
                `${this._settings.apiEndpoint}/assets?${utils.serializeParams(params)}`,
                this._settings.headers
            )
                .then(response => resolve(response))
                .catch(error => reject(error));
    ...
    createAsset(asset = {}, events = []) {
        ...
        return new Promise((resolve, reject) => {
          ...
          (assetSequenceNumber + 1) % 1000000),
            ...
            postRequest(
                `${this._settings.apiEndpoint}/assets`,
                this._settings.headers,
                params
            )
                .then(async assetRes => {
